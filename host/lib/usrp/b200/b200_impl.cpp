...
static device_addrs_t b200_find(const device_addr_t& hint)
{
    ...
    /* microphase */
    UHD_LOGGER_INFO("ANTSDR") << "Search Microphase ANTSDR E310.";
    /* connect the device from ethernet "addr=" */
    udp_simple::sptr udp_transport;
    device_addr_t find_e310;
    find_e310["addr"] = "192.168.2.25";
    try {

        udp_transport = udp_simple::make_broadcast(
            find_e310["addr"], BOOST_STRINGIZE(MICROPHASE_E310_UDP_CTRL_PORT));
    ...
    try {
        ...
        UHD_LOGGER_ERROR("Microphase E310") << "E310 Network discovery error" << ex.what();
    } catch (...) {
        UHD_LOGGER_ERROR("Microphase E310") << "E310 Network discovery unkonwn error";
    }
    ...
    , _tick_rate(0.0) // Forces a clock initialization at startup
{
     ...
     * that calling update_bandsel calls update_gpio_state(). */
        ...
        ////////////////////////////////////////////////////////////////////
        std::string product_name = "ANTSDR-E310";
